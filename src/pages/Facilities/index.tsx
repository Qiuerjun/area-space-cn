    ...
    <div className="min-h-screen p-8">
      ...
      >
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-3xl font-mono text-cyan-400">站点设施</h1>
        </div>
      ...
      >
        {serverStats.map((stat, index) => (
          <motion.div
            ...
            className="bg-[#0d0d15] border border-cyan-500/20 p-6 text-center transition-all"
          >
            <stat.icon className="w-8 h-8 text-cyan-400 mx-auto mb-3" />
            <p className="text-gray-400 font-mono text-sm mb-2">{stat.label}</p>
      ...
      <motion.section
        ...
        className="mb-8 bg-gradient-to-r from-[#0a0a1a] to-[#0d1525] border border-cyan-500/30 p-8"
      >
      ...
      <motion.section
        ...
        className="mb-8 bg-[#0d0d15] border border-cyan-500/20 p-6"
      >
        <h2 className="text-lg font-mono text-cyan-400 mb-4 pb-4 border-b border-cyan-500/20">
          CB意识体能力
        </h2>
        <div className="space-y-4">
          <div className="bg-[#0a0a0f] border border-cyan-500/10 p-4">
            <h3 className="text-cyan-400 font-mono text-sm mb-2">网络行动</h3>
          ...
          {serverCapabilities.map((cap, index) => {
            ...
            return (
              <motion.div
                ...
                className={`${colors.bg} border ${colors.border} p-6 transition-all`}
              >
                <div className="flex items-start gap-4">
                  <div
                    className={`w-12 h-12 ${colors.bg} border ${colors.border} flex items-center justify-center flex-shrink-0`}
                  >
                    <cap.icon className={`w-6 h-6 ${colors.text}`} />
                  </div>
                  <div>
                    <h3 className={`${colors.text} font-mono text-lg mb-2`}>
                      {cap.name}
                    </h3>
